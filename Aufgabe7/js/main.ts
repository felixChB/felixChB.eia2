...
namespace js_to_html {
    ...
    function init(_event: Event): void {
        ...
        for (let i: number = 0; i < fieldsets.length; i++) {
            ...
            document.getElementById("bp").addEventListener("click", writeUrl);
        }
    ...
    function handleChange(_event: Event): void {
        ...
        for (let i: number = 0; i < allBoxes.length; i++) {
            if (allBoxes[i].checked == true) {
                ...
                } else if (allBoxes[i].name == "WaffeloderBecher") {
                    let ziel = document.createElement("li");
                    ziel.innerHTML = `${allBoxes[i].id}`;
                    document.getElementById("wob").appendChild(ziel);
                } else if (allBoxes[i].name == "Lieferoption") {
                    let ziel = document.createElement("li");
                    ziel.innerHTML = `${allBoxes[i].id}`;
    ...
    function writeUrl(): void {
        ...
        for (let i: number = 0; i < allBoxes.length; i++) {
            if (allBoxes[i].type == "number")
                if (Number(allBoxes[i].value) > 0) {
                    writtenUrl += `${allBoxes[i].id}=${allBoxes[i].value}&`;
                }
            if (allBoxes[i].type == "checkbox") {
                if (allBoxes[i].checked == true) {
                    writtenUrl += `${allBoxes[i].id}=${allBoxes[i].value}&`;
            ...
            if (allBoxes[i].name == "WaffeloderBecher") {
                if (allBoxes[i].checked == true) {
                    writtenUrl += `${allBoxes[i].name}=${allBoxes[i].value}&`;
                }
            }
            if (allBoxes[i].name == "Lieferoption") {
                if (allBoxes[i].checked == true) {
        ...
        sendRequestWithCustomData(writtenUrl);
    }
